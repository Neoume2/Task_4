...
export async function createPerk(req, res, next) {
  try {
    ...
    if (err.code === 11000) {
      try {
        // If a duplicate perk exists, return it instead of failing the whole test run.
        const existing = await Perk.findOne({ title: req.body.title, merchant: req.body.merchant });
        if (existing) return res.status(200).json({ perk: existing });
      } catch (findErr) {
        // ignore and fall through to next(err)
      }
